    ...
    <div style={{ padding: 20, fontFamily: 'system-ui' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <button onClick={() => navigate('/rent')} style={{
          padding: 16, background: '#2AABEE', color: '#fff', border: 'none',
          borderRadius: 12, fontSize: 16, cursor: 'pointer'
        }}>🗓️ Аренда студии</button>
        <button onClick={() => navigate('/packages')} style={{
          padding: 16, background: '#2AABEE', color: '#fff', border: 'none',
          borderRadius: 12, fontSize: 16, cursor: 'pointer'
        }}>📦 Пакеты часов</button>
        <button onClick={() => navigate('/mixing')} style={{
          padding: 16, background: '#2AABEE', color: '#fff', border: 'none',
          borderRadius: 12, fontSize: 16, cursor: 'pointer'
        }}>🎚️ Сведение / Под ключ</button>
        <button onClick={() => navigate('/question')} style={{
          padding: 16, background: '#2AABEE', color: '#fff', border: 'none',
          borderRadius: 12, fontSize: 16, cursor: 'pointer'
